...
def get_images(path):
    ...
    image_paths = [os.path.join(path, f) for f in os.listdir(path) if not f.endswith('.result')]
...
path2 = 'D:\\photo'
image_paths = [os.path.join(path2, f) for f in os.listdir(path2) if f.endswith('.result')]
